...
class Pylontech:
    ...
    def __init__(self, serial_port='/dev/cu.usbserial-FT65F5C4', baudrate=9600):
        self.s = serial.serial_for_url(sys.argv[1], timeout=2)
#        self.s = serial.Serial(serial_port, baudrate, bytesize=8, parity=serial.PARITY_NONE, stopbits=1, timeout=2)
    ...
    def get_values(self):
        self.send_cmd(int(sys.argv[2]), 0x42, b'FF')
        f = self.read_frame()
        ...
        data = {
            "number_of_cells": d.Module[0].NumberOfCells,
            "cell_voltages": d.Module[0].CellVoltages[:],
            "number_of_temperatures": d.Module[0].NumberOfTemperatures,
            "avg_bms_temperature": d.Module[0].AverageBMSTemperature,
            "temperatures": d.Module[0].GroupedCellsTemperatures[:],
            "current": d.Module[0].Current,
            "voltage": d.Module[0].Voltage,
            "power": d.Module[0].Power,
            "remaining_capacity": d.Module[0].RemainingCapacity,
            "total_capacity": d.Module[0].TotalCapacity,
            "cycle_number": d.Module[0].CycleNumber,
